...
pub(crate) mod actor {
    // The heartbeat actor demonstrates the fundamental actor pattern
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Actors use standard log macros (info!, warn!, error!) which are
    ...
    // The system continues running until one actor calls request_shutdown(),  
...
fn build_graph(graph: &mut Graph) {
    ...
    graph.actor_builder()
        ...
        // This provides real-time performance metrics without performance overhead
        .with_mcpu_avg()
